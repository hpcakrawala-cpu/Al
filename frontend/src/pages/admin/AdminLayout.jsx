...
import { LayoutDashboard, FileText, ReceiptText, FilePlus2, LogOut, ExternalLink, FileSignature, ClipboardList, Wallet, Boxes, Users } from "lucide-react";
...
const groups = [
  ...
  },
  {
    title: "Operasional",
    links: [
      { to: "/admin/spk", label: "SPK per Anggota", icon: ClipboardList },
      { to: "/admin/kas", label: "Uang Kas", icon: Wallet },
      { to: "/admin/barang", label: "Barang Masuk/Keluar", icon: Boxes },
  ...
  },
  {
    title: "Dokumen",
    links: [
      { to: "/admin/dokumen", label: "Invoice & Kwitansi", icon: FileText },
  ...
  return (
    <div className="min-h-screen bg-[#0a1120] flex">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-[#060d1a] border-r border-[#4facfe]/15 p-6 fixed inset-y-0">
        ...
        <nav className="mt-8 space-y-5 flex-1 overflow-y-auto">
          {groups.map((g) => (
            <div key={g.title}>
              <div className="px-4 text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">{g.title}</div>
              <div className="space-y-1">
                {g.links.map((l) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-4 py-2.5 rounded-xl font-medium text-sm transition-brand ${
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 bg-[#060d1a] border-b border-[#4facfe]/15 px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex gap-1 overflow-x-auto">
          {allLinks.map((l) => (
            <NavLink key={l.to} to={l.to} end={l.end} className={({ isActive }) => `p-2.5 rounded-lg shrink-0 ${isActive ? "bg-[#2f7bff] text-white" : "text-slate-300"}`}>
              <l.icon size={20} />
            </NavLink>
          ))}
          <button onClick={logout} className="p-2.5 text-slate-300 shrink-0"><LogOut size={20} /></button>
        </div>
      </div>
